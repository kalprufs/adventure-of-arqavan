...
function update(timestamp) {
    ...
    if (game.character.isMoving && !game.character.isJumping) {
        game.character.animationFrame = (game.character.animationFrame + config.character.walkAnimationSpeed) % config.character.frameCount; // Use frameCount here
    } else {
...
function render() {
    ...
    // Calculate source Y based on current animation frame
    const characterFrameHeightInSheet = config.character.spriteSheetHeight / config.character.frameCount;
    const characterSourceY = Math.floor(game.character.animationFrame) * characterFrameHeightInSheet + config.character.sourceOffsetY;

    // Draw character using source parameters for animation
    drawSprite(characterSprite, game.character.x, game.character.y, config.character.width, config.character.height,
               config.character.sourceOffsetX, characterSourceY, config.character.sourceFrameWidth, config.character.sourceFrameHeight);
...
function handleKeyUp(e) {
    ...
    // Removed problematic lines here; movement state is now managed in the update function
}
